...
if (localStorage.getItem(ONLOGIN) !== "null") {
  ...
  location.href = "/html/header/3 login.html";
}
}
...
if (parseWishName[parseWishName.indexOf(productName.innerText)] == productName.innerText ) {
...
function onCart (event) {
  ...
  if(localStorage.getItem(CARTNAME) == null) {
    ...
    localStorage.setItem(CARTONEPRODUCTPRICE, JSON.stringify(cartOneProdutPrice));
    location.reload();
  }
  else{
    saveCart();
...
function cartIntoLocalStorage (saveCart) {
      ...
      localStorage.setItem(CARTONEPRODUCTPRICE, JSON.stringify(cartOneProdutPrices));
  }
