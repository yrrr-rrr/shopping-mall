...
function saveLogin (event) {
  ...
  fetch("https://shoppingmall.fly.dev/signup", {
    method: "POST",
    headers: {
      "content-type": "application/json ; charset=utf-8", "Accept" : "*/*", "Access-Control-Allow-Origin" : "https://shoppingmallserver.fly.dev/"
    },
    body: JSON.stringify(
      {
        "id" : loginId.value,
  ...
  }).then((response) => response.json().then((data) => localStorage.setItem(SAVEUSERNAME, data.name)))
